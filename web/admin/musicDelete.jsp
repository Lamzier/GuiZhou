...
<%
    ...
    try {
        ...
        out.println("window.location = \"" + request.getContextPath() + "/admin/musicList.jsp" + "\"</script>");
        return;
    }
    Map<String , Object> musicInfo = Query.getMusicInfo(id);
    if(musicInfo == null || musicInfo.size() <= 0){
        out.println("<script> alert(\"音乐不存在！！！\")");
        out.println("window.location = \"" + request.getContextPath() + "/admin/musicList.jsp" + "\"</script>");
        return;
    }
    String pathString = (String) musicInfo.get("pathString");
    if(!Update.deleteMusic(id , pathString)){
